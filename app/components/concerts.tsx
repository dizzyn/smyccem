...
export default function Concerts() {
  const concertsArr = useMemo(
    () => [
      {
        date: '19/4/2023',
        place: 'Kavárna Unijazz, Praha',
      },
      {
        date: '20/4/2023',
        place: 'Kavárna, co hledá jméno, Praha',
      },
      {
        date: '21/4/2023',
        place: 'Mirotické setkání loutek, Mirotice',
      },
    ],
    []
  );

  const [currentConcert, setCurrentConcert] = useState(concertsArr[0]);
    ...
    const timer = setInterval(() => {
      ...
      setTimeout(() => {
        setIndex((prevIndex: number) => {
          const newIndex = (prevIndex + 1) % concertsArr.length;
          setCurrentConcert(concertsArr[newIndex]);
          return newIndex;
        });
        setFade(true);
      }, 500);
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-black md:bg-transparent items-center md:gap-2 text-white md:items-end md:justify-center px-8 py-2 md:pr-16 fixed bottom-0 left-0 right-0 md:relative">
      ...
      >
        {currentConcert.date} - {currentConcert.place}
      </Link>
      <div className="flex">
        <Link
